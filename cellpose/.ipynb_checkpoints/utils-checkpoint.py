...
def grow_masks(masks,growth, num_neighbors = 30):
    ...
    for _ in range(growth):
        for i in range(num_masks):
            ...
            if len(pix_X) > 0 and len(pix_Y)>0:
                masks[pix_Y, pix_X] = i + 1

    return masks    

##
def filter_masks_without_nuclei(pred_mask, nuclei_masks):
    processed_pred_masks = np.zeros_like(pred_mask)
    logical_nuclei_mask = nuclei_masks>0
    pred_cell_count = 1
    for i in range(1, np.max(pred_mask)+1):
        if np.any(np.logical_and(pred_mask == i, logical_nuclei_mask)):
            processed_pred_masks[pred_mask == i] = pred_cell_count
            pred_cell_count += 1
            
    return processed_pred_masks