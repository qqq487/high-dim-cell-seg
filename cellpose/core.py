...
import fastremap
from . import transforms, dynamics, utils, plot, metrics, losses
...
#         slc = [slice(0, self.nclasses) for n in range(imgs.ndim)] # changed from imgs.shape[n]+1 for first slice size 
    ...
    def _train_step(self, x, lbl):
        ...
        self.net.train()
        y = self.net(X)[0]
        loss, vec_loss, lbl_loss_1, lbl_loss_2 = self.loss_fn(lbl,y) ## 
        loss.backward()
        train_loss = loss.item()
        self.optimizer.step()
        train_loss *= len(x)
        return train_loss, vec_loss, lbl_loss_1, lbl_loss_2 ##
